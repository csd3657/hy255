...
typedef struct grid_s {
  int is_unique;      /* if 1 after solving, puzzle has unique solution */
  struct { 
    int value;       /* value of element i,j */
    struct {         
      int possible_choices;   /* number of possible choices for element i,j */ 
  ...
  } elements[9][9];    /* sudoku grid elements; 0<=i,j<=9 */
} Grid_T;
